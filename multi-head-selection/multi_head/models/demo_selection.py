...
class MultiHeadSelection(nn.Module):
    ...
    def masked_BCELoss(self, mask, selection_logits, selection_gold):
        # 一顿操作mask, 输入的Mask = [16, seq_len] = [16, 100]
        # 目标是希望mask = [16, 100, 50, 100]
        selection_mask = (mask.unsqueeze(2) * mask.unsqueeze(1)).unsqueeze(2)
        # selection_mask = [16, 100, 1, 100]
        selection_mask = selection_mask.expand(-1, -1, len(self.relation_vocab), -1)

        # 计算损失值, 二分类的交叉熵损失
        loss = F.binary_cross_entropy_with_logits(selection_logits, selection_gold, reduction='none')

        # 还得掩码操作
        loss = loss.masked_select(selection_mask).sum()

        # mask.sum()代表有多少个有效的三元组
        loss /= mask.sum()
    ...
    def forward(self, sample, is_train, is_predict=False):
        ...
        # (32, 200, 200, 100) ,  (50, 100) -> (32, 200, 50, 200)
